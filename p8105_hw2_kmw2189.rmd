...
```{r, uploading dataset for problem 3}
baby_names = read_csv(file = "./data/popular_baby_names.csv") %>%
  janitor::clean_names() %>%
  rename(year = year_of_birth , name = childs_first_name) %>% 
  mutate(
    ethnicity = recode("ASIAN AND PACI" = "ASIAN AND PACIFIC ISLANDER", "BLACK NON HISP" = "BLACK NON HISPANIC", "WHITE NON HISP" = "WHITE NON HISPANIC"
  )
